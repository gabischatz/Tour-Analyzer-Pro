...
function setDisabledSafe(el, disabled) {
...
function addListenerSafe(el, eventName, handler) {
...
if (!els || !els.loadDemoBtn) return;
const btn = els.loadDemoBtn;
const originalText = btn.textContent;
btn.disabled = true;
btn.textContent = '⏳ Demo wird geladen...';

try {
    const demoUrl = new URL('./demo/Bad-Langensalza-To-Kirchheiligen-and-Return.gpx', window.location.href).href;
    logEvent('Demo-Route wird geladen: ' + demoUrl);
    const response = await fetch(demoUrl, { cache: 'no-store' });
    if (!response.ok) throw new Error('HTTP ' + response.status + ' beim Laden der Demo-Route.');
    ...
    if (rawPoints.length < 2) throw new Error('Die Demo-GPX enthält zu wenige Trackpunkte.');
    await debugBreakpoint('fileSelected', 'Demo-GPX geladen', {
        datei: 'Bad-Langensalza-To-Kirchheiligen-and-Return.gpx',
        rawPoints: rawPoints.length
    });
    await processTrack(rawPoints, author);
    setStatus('Demo-Route wurde geladen und verarbeitet.', 'success');
} catch (err) {
    console.error(err);
    const msg = err && err.message ? err.message : String(err);
    setStatus('Fehler beim Laden der Demo-Route: ' + msg, 'error');
    logEvent('Fehler beim Demo-Laden: ' + msg, 'error');
} finally {
    btn.disabled = false;
...
function bindBreakpointControls() {
    ...
    function bindEvents() {
        addListenerSafe(els.fileInput, 'change', onFileSelected);
        addListenerSafe(els.reloadOsmBtn, 'click', loadOsmRoutesForCurrentTrack);
        addListenerSafe(els.extractBtn, 'click', handleSegmentCalculationRequest);
        addListenerSafe(els.exportBtn, 'click', exportGeoJSONFile);
        addListenerSafe(els.exportCombinedBtn, 'click', exportGeoJSONFile);
        addListenerSafe(els.exportFinalRouteBtn, 'click', exportFinalRouteGeoJSONFile);
        addListenerSafe(els.exportFinalRouteGpxBtn, 'click', exportFinalRouteGpxFile);
        addListenerSafe(els.exportPiecesBtn, 'click', exportPiecesGeoJSONFile);
        addListenerSafe(els.exportOsmBtn, 'click', exportOsmGeoJSONFile);
        addListenerSafe(els.exportPointsBtn, 'click', exportAutoPointsGeoJSONFile);
        addListenerSafe(els.exportRawOverpassBtn, 'click', exportRawOverpassFile);
        addListenerSafe(els.exportDebugBtn, 'click', exportDebugBundleFile);
        addListenerSafe(els.clearLogBtn, 'click', clearLogPanel);
        addListenerSafe(els.resetBtn, 'click', resetAll);
        addListenerSafe(els.toggleSidebarBtn, 'click', toggleSidebar);
        addListenerSafe(els.presetAllBtn, 'click', applyPresetAllLayers);
        addListenerSafe(els.presetResultBtn, 'click', applyPresetResultLayers);
        addListenerSafe(els.presetDebugBtn, 'click', applyPresetDebugLayers);
        addListenerSafe(els.loadDemoBtn, 'click', loadDemoRoute);
    }
    ...
    function setExportButtonsEnabled(enabled) {
        const val = !enabled;
        setDisabledSafe(els.exportBtn, val);
        setDisabledSafe(els.exportCombinedBtn, val);
        setDisabledSafe(els.exportFinalRouteBtn, val);
        setDisabledSafe(els.exportFinalRouteGpxBtn, val);
        setDisabledSafe(els.exportPiecesBtn, val);
        setDisabledSafe(els.exportOsmBtn, val);
        setDisabledSafe(els.exportPointsBtn, val);
        setDisabledSafe(els.exportRawOverpassBtn, val);
        setDisabledSafe(els.exportDebugBtn, val);
    }
    ...
    async function processTrack(rawPoints, author) {
        ...
        setDisabledSafe(els.reloadOsmBtn, false);
        setDisabledSafe(els.extractBtn, false);
        setExportButtonsEnabled(true);
        setValueSafe(els.pointA, '');
        setValueSafe(els.pointB, '');
    ...
    function drawTrackAnalysis(analysis) {
        ...
        setTextSafe(els.statTotal, formatKm(analysis.allDistance));
        setTextSafe(els.statOutbound, formatKm(analysis.outboundDistance));
        setTextSafe(els.statReturn, formatKm(analysis.returnDistance));
        setTextSafe(els.statCommon, formatKm(analysis.commonDistance));
        setTextSafe(els.statSegment, state.autoPoints.length + ' Punkte');
        syncVisibilityFromUi();
    }
    ...
    async function loadOsmRoutesForCurrentTrack() {
        ...
        const relationIdRaw = els.relationIdInput && typeof els.relationIdInput.value === 'string' ? els.relationIdInput.value.trim() : '';
        ...
        try {
            ...
            setTextSafe(els.statRoutes, ways.length + ' Wege / ' + state.osmGraph.nodeList.length + ' Knoten');
            syncVisibilityFromUi();
            ...
            console.error(err);
            setTextSafe(els.statRoutes, '0 Wege');
    ...
    function setManualPoint(type, lat, lng) {
        ...
        if (type === 'A') setValueSafe(els.pointA, snapped.lat.toFixed(6) + ',' + snapped.lng.toFixed(6) + ' | Index ' + snapped.index);
        else setValueSafe(els.pointB, snapped.lat.toFixed(6) + ',' + snapped.lng.toFixed(6) + ' | Index ' + snapped.index);
    ...
    function calculateAutomaticRoutePieces(fromButton) {
        ...
        setTextSafe(els.statSegment, state.autoPoints.length + ' Punkte / ' + state.autoRoutePieces.length + ' Stücke');
        rebuildDynamicExportFeatures();
        if (fromButton) {
    ...
    function resetAll() {
        ...
        clearAllMarkers();
        resetMapLayersOnly();
        setValueSafe(els.fileInput, '');
        setValueSafe(els.pointA, '');
        setValueSafe(els.pointB, '');
        setDisabledSafe(els.reloadOsmBtn, true);
        setDisabledSafe(els.extractBtn, true);
        setExportButtonsEnabled(false);
        setTextSafe(els.statTotal, '–');
        setTextSafe(els.statOutbound, '–');
        setTextSafe(els.statReturn, '–');
        setTextSafe(els.statCommon, '–');
        setTextSafe(els.statSegment, '–');
        setTextSafe(els.statRoutes, '–');
